...
![Version](https://img.shields.io/badge/version-1.0.2-0A7BBB)
...
- [Dependencies](#dependencies)
  ...
  - [Manual installation examples](#manual-installation-examples)
- [Router preparation](#router-preparation)
- [Dependency management behavior](#dependency-management-behavior)
- [Quick start](#quick-start)
- [Command-line usage](#command-line-usage)
- [Output files](#output-files)
- [Changing the PDF table style](#changing-the-pdf-table-style)
...
`srm-dhcp-export` is an interactive Bash bundle that connects to a Synology router running SRM, reads static DHCP reservations from `/etc/dhcpd/dhcpd.conf`, downloads a temporary raw list through the existing SSH session, normalizes the data, and creates:
...
- SSH-streamed downloads that remain compatible with older SRM servers lacking an SFTP subsystem.
...
| OpenSSH client (`ssh`) | Router connection and SSH-streamed file transfer |
...
- complete exporter runs with an SSH test double, including transfer and cleanup failures.
...
### `Connection closed` during the download

Version 1.0.2 and later do not invoke `scp`. The reservation list is streamed through the already authenticated SSH connection because recent OpenSSH versions make `scp` use SFTP by default, while older SRM SSH servers may not expose an SFTP subsystem. If this message still appears, verify that the installed script reports version 1.0.2 or later with `./srm-dhcp-export.sh --version`.
