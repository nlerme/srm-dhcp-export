...
readonly DEFAULT_HOST="192.168.0.1"
...
Examples:
  ./srm-dhcp-export.sh
  ./srm-dhcp-export.sh --host 192.168.0.1 --output reports/devices.pdf
...
collect_missing_dependencies() {
    ...
    if ! command_exists ssh; then
        MISSING_COMPONENTS+=("OpenSSH client (ssh)")
    fi
...
build_package_list() {
    ...
    for component in "${MISSING_COMPONENTS[@]}"; do
        case "$manager:$component" in
            apt-get:'OpenSSH client (ssh)') add_package openssh-client ;;
            ...
            dnf:'OpenSSH client (ssh)'|yum:'OpenSSH client (ssh)') add_package openssh-clients ;;
            ...
            pacman:'OpenSSH client (ssh)') add_package openssh ;;
            ...
            zypper:'OpenSSH client (ssh)') add_package openssh ;;
            ...
            apk:'OpenSSH client (ssh)') add_package openssh-client ;;
            ...
            brew:'OpenSSH client (ssh)') add_package openssh ;;
...
# Stream the file through the existing SSH session instead of invoking scp.
# Modern OpenSSH clients use SFTP for scp by default, but older SRM SSH servers
# may not provide an SFTP subsystem and close the connection immediately.
REMOTE_DOWNLOAD_COMMAND="if [ ! -r '$REMOTE_FILE' ]; then printf '%s\n' 'Remote reservation list is not readable: $REMOTE_FILE' >&2; exit 43; fi; cat '$REMOTE_FILE'"
if ! ssh "${SSH_COMMON_OPTIONS[@]}" "$SSH_DESTINATION" "$REMOTE_DOWNLOAD_COMMAND" > "$RAW_FILE"; then
    abort "The reservation list download over SSH failed."
fi
